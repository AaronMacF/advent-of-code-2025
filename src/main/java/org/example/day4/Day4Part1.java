...
public class Day4Part1 implements Day {
    ...
    public void run() {
        this.populateGrid();
        long rolls = grid.stream().mapToLong(cells -> cells.stream().filter(this::isCellAccessible).count()).sum();
        System.out.println("Accessible rolls: " + rolls);
    }

    private void populateGrid() {
        List<String> input = FileUtilities.readPuzzleInput("day4.txt");
    ...
    private boolean isCellAccessible(Cell cell) {
        System.out.println("Checking cell (" + cell.xIndex + "," + cell.yIndex + ")");
        return cell.isRoll && this.numberOfNeighbourRolls(cell) < 4;
    }
